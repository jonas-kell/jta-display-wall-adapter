...
pub async fn run_network_task(
    ...
    rx_from_com_port: InactiveReceiver<IDCaptureMessage>,
    ...
    loop {
        ...
        .await
        {
            Ok(Ok((inbound, client_addr))) => {
                ...
                let mut deserializer: Framed<_, MessageToIdcaptureServer, IDCaptureMessage, _> =
                    Framed::new(
                        FramedRead::new(read_half, make_json_exchange_codec()),
                        Json::<MessageToIdcaptureServer, IDCaptureMessage>::default(),
                    );
                let mut serializer: Framed<_, MessageToIdcaptureServer, IDCaptureMessage, _> =
                    Framed::new(
                        ...
                        loop {
                            ...
                            .await
                            {
                                ...
                                Ok(Some(Ok(mes))) => match mes {
                                    // here we can define incoming messages if we ever need to tell the idcapture server things
                                },
                            }
                        }
                        Ok::<_, String>(())
